...
namespace CrazyKTV_SongMgr
{
    class SongAnalysis
    {
        ...
        public static void SongInfoAnalysis(string file, List<string> SongLangKeyWordList, List<string> SingerTypeKeyWordList, List<string> SongTrackKeyWordList)
        {
            ...
            SongSongTypeList = null;
            SongSongTypeLowCaseList.Clear();
            SongSongTypeLowCaseList = null;
            list.Clear();
            list = null;
            ...
            if (SongSongName == "")
            {
                ...
                if (FileNameRemoveList.Count > 0)
                {
                    ...
                    {
                        str = Regex.Replace(str, @"([\{\(\[｛（［【](.+)?)" + Regex.Escape(RemoveStr) + @"([】］）｝\]\)\}])", "$1$3", RegexOptions.IgnoreCase);
                ...
                switch (SongSinger)
                {
                    ...
                    case "張懸":
                        if (SongLang == "兒歌")
                        {
                            ...
                            else if (FileStrCount == 1 && SongSongName == "兒歌")
                            {
                                SongLang = "國語";
                            }
                        }
                    ...
                    case "莫文蔚":
                        if (SongLang == "台語")
                        {
                            ...
                            else if (FileStrCount == 1 && SongSongName == "台")
                            {
                                SongLang = "國語";
                            }
                        }
